...
from keras.layers import Conv2D, Add, ZeroPadding2D, UpSampling2D, Concatenate, MaxPooling2D, GlobalAveragePooling2D, \
    Activation, SeparableConv2D, DepthwiseConv2D, Input, Add, Lambda, Reshape, Multiply, Dense, Dropout
...
def ghostBottleneck(x, hidden_dim, out_dim, kernel_size=3, ratio=2, strides=1, use_se=False,
    ...
    hidden = GhostModule(x, hidden_dim, kernel_size=1, ratio=ratio, kernel_initializer=kernel_initializer,
                         kernel_regularizer=kernel_regularizer)
    if strides == 2:
        hidden = DepthwiseConv2D(kernel_size, strides=2, padding='SAME', depthwise_regularizer=kernel_regularizer,
                                 depthwise_initializer=kernel_initializer)(hidden)
    if use_se:
        # pass
        hidden = SELayer(hidden, hidden_dim)
    res = GhostModule(hidden, out_dim, kernel_size=1, ratio=ratio, relu=False, kernel_initializer=kernel_initializer,
                      kernel_regularizer=kernel_regularizer)
    shortcut = x
    if strides == 2:
...
def my_ghostBottleneck(x, hidden_dim, out_dim, kernel_size=3, ratio=2, strides=1, kernel_initializer='he_normal',
                       kernel_regularizer=None, use_se=False):
    assert strides in [1, 2]
    input_dim = int(x.shape[3])
    hidden = GhostModule(x, hidden_dim, kernel_size=1, kernel_initializer=kernel_initializer,
                         kernel_regularizer=kernel_regularizer, ratio=ratio)
    if strides == 2:
        hidden = DepthwiseConv2D(kernel_size, strides=2, kernel_initializer=kernel_initializer,
                                 depthwise_regularizer=kernel_regularizer, padding='SAME')(hidden)
    if use_se:
        # pass
        hidden = SELayer(hidden, hidden_dim)
    res = GhostModule(hidden, out_dim, kernel_size=kernel_size if strides == 1 else 1,
                      kernel_initializer=kernel_initializer, kernel_regularizer=kernel_regularizer, ratio=ratio,
                      relu=False)
    shortcut = x
    if strides == 2:
        shortcut = DepthwiseConv2D(3, strides=2, padding='SAME')(shortcut)
...
def ghostnet(mul=1, ratio=2,class_num=9):
    inputs = Input(shape=(384, 384, 3))
    x = ghost_body(inputs, mul=mul, ratio=ratio)
    x = Conv2D(_make_divisible(960 * mul), 3, strides=1, padding="SAME", use_bias=False)(x)
...
if __name__ == '__main__':
    ...
    # inputs = Input(shape=(384, 384, 3))
    # model = Model(inputs=inputs, outputs=ghost_body(inputs,mul=0.5))
    model =ghostnet()
    model.summary()
    print("ghostnet have", len(model.layers), "layers")
