...
from yolo3.model import dyolo_eval, dyolo_body
...
class DYOLO(object):
    ...
    def generate(self):
        ...
        try:
            self.dyolo_model = load_model(model_path, compile=False)
        except:
            self.dyolo_model = dyolo_body(Input(shape=(None, None, 3)), num_anchors // 1, num_classes)
            self.dyolo_model.load_weights(self.model_path)  # make sure model, anchors and classes match
        else:
            assert self.dyolo_model.layers[-1].output_shape[-1] == \
        ...
        if self.gpu_num >= 2:
            self.dyolo_model = multi_gpu_model(self.dyolo_model, gpus=self.gpu_num)
        boxes, scores, classes = dyolo_eval(self.dyolo_model.output, self.anchors,
...
def detect_video(yolo, video_path, output_path=""):
    ...
    yolo.close_session()
def main():
    dyolo_model = dyolo_body(Input(shape=(384, 384, 3)), 3, 10)
    print("dyolo_model have",len(dyolo_model.layers),"layers")
    dyolo_model.summary()
